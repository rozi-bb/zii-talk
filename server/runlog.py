...
async def sync(run_id: UUID, topic_id: str, situation: str, model: str, turns: list[Turn]) -> Saved | None:
    answers = sum(t.role == "me" for t in turns)
    async with (await db.pool()).connection() as conn, conn.transaction():
        if not await _lock_run(conn, run_id):
            ...
            await conn.execute(
                """
                INSERT INTO test_runs (id, topic_id, attempt_no, situation, model, started_at, ended_at)
                VALUES (%s, %s,
                        (SELECT coalesce(max(attempt_no), 0) + 1 FROM test_runs WHERE topic_id = %s),
                        %s, %s, %s, now())
                ON CONFLICT (id) DO NOTHING
                """,
                (run_id, topic_id, topic_id, situation, model, turns[0].at),
            )
...
async def save_reply(run_id: UUID, seq: int, text: str, correction: dict[str, str] | None) -> None:
    async with (await db.pool()).connection() as conn, conn.transaction():
        if not await _lock_run(conn, run_id):
...
async def rewind(run_id: UUID, keep: int) -> Saved | None:
...
async def _lock_run(conn: AsyncConnection[Any], run_id: UUID) -> bool:
    cur = await conn.execute("SELECT 1 FROM test_runs WHERE id = %s FOR UPDATE", (run_id,))
